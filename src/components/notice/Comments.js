...
const Comments = (props) => {
  ...
  // Comment Create
  const create = async (textarea) => {
    if (validation(textarea.value)) {
      await props.create(textarea.value);
      setMsg('');
    }
  }
      ...
      <>
        {
          ...
          ? <div>
            <h4>Comments</h4>
            <CommentAllow textareaRef={ textareaValue } msg={ msg } setMsg={ setMsg } />
            <div className="col-md-12">
              <button type="button" className="btn btn-outline-primary btn-sm m-1" onClick={ () => create(textareaValue.current) } >Submit</button>
            </div> 
            </div>
